...
The reference implementation this page follows, and the one to read first if you want a from-scratch, independently reviewed treatment of the paper rather than this site's own live-demo pipeline, is the standalone [MitteldorfWilson2000](https://github.com/doesburg11/MitteldorfWilson2000) repository — see [An Independent From-Scratch Replication](#an-independent-from-scratch-replication) below.
...
## An Independent From-Scratch Replication

A second, standalone implementation of this same paper exists at [MitteldorfWilson2000](https://github.com/doesburg11/MitteldorfWilson2000), built independently of the `EvolvedCooperation` module this page's replay and figures are drawn from, and reviewed by an independent model (Codex) before being committed — the review caught two real bugs (a swath-culling area-sizing error on non-square grids, and a censored-run reporting gap), both fixed and documented rather than quietly patched.

Where it differs from this page's own integration is the outcome it measures. This page reports coexistence probability and occupied fraction at a fixed generation cutoff (`step 1000` in the Culling Experiment above). `MitteldorfWilson2000` instead measures fixation probability directly, via dose-response sweeps over `b` at fixed `c`, and its `RESULTS.md` is explicit about a limitation that reporting coexistence probability alone can obscure: near the classical `b = 5c` boundary, genuine fixation of one type is slow enough that most trials never resolve within a practical generation budget, so most of its own reported numbers are properly read as "which type holds the numeric majority at cutoff," not "which type actually won." One condition in that repository's sweep — 95% uniform-culling severity — did reach genuine fixation in every trial, and is flagged there as the one number that can be trusted as an outright win rate rather than a majority-at-cutoff proxy.

That repository also ships its own interactive `pygame` visualizer (`run_pygame.py`), driving the same tested model code live with keyboard controls for `b`, `c`, `harshness`, `disease`, and the periodic disturbance schedule — a Python-native counterpart to the browser replay above.

If you want the paper's equations and this page's own experimental claims verified from a second, independent codebase before trusting them, that repository is the one to read.
...
- `MitteldorfWilson2000`. Independent, from-scratch, Codex-reviewed replication of the same paper — the reference implementation this page follows. GitHub. https://github.com/doesburg11/MitteldorfWilson2000
- `EvolvedCooperation`. This site's own model integration, feeding the replay and figures above. *ecological_models/spatial_altruism* module directory. GitHub. https://github.com/doesburg11/EvolvedCooperation/tree/main/ecological_models/spatial_altruism
