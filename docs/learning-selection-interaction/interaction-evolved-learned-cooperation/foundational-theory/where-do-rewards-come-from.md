...
**First real-scale result (2026-09-12):** one seed, the full 1,000,000-step budget, survived to completion — 5.16 million logged agent lifetimes. Evolution weighted `health_norm` and `energy_norm` roughly 3–4× more heavily than any of the five other observation channels, despite neither one having any meaningful *raw* correlation with realized `offspring_count` on its own (every channel sits under 0.01 correlation with raw offspring count). That's the same qualitative pattern as the paper's own Hungry-Thirsty result: the evolved reward concentrates on dense, always-available internal state — health and energy, this world's closest analogue to "food reward" — rather than on whatever single channel happens to best predict the fitness event directly.

Two honest caveats before this counts as confirmed rather than suggestive: single-channel correlation isn't the same as causal irrelevance (fitness depends on the whole policy learned against all channels jointly, not any one channel in isolation), and this is one seed — this project's own standard is 3+ before treating a pattern as real. Two more seeds are running now to check replication. Full numbers and the fixed methodological caveats live in the module's [`RESULTS.md` §17](https://github.com/doesburg11/PredPreyGrass/blob/main/predpreygrass/evolutionary/eco_evolutionary_erl_baldwin/RESULTS.md).
