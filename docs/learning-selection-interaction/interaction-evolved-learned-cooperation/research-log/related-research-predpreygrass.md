...
A visual, foldable overview of the concepts covered on the [Interaction Evolved-Learned Cooperation](/learning-selection-interaction/theory) page. **Nature vs Nurture** (plus a **Hybrids** branch) is the top-level split. Nature, Life-time Learning, and Hybrids each split further into **Individual** and **Multi-Agent** contexts; Cultural Inheritance, Developmental & Epigenetic Inheritance, and Material & Economic Inheritance round out Nurture beyond learning — these three are conceptual/literature branches rather than algorithms this project currently models. Every named concept or algorithm is its own node. Nested beneath it, each supporting work gets a pair of sibling nodes: a type label (**Concept** for foundational/theoretical work, **Computational** for formal, empirical, or simulation-based work) and the paper or book itself (shown in white, underlined) — rather than packing name, type, and citation onto one line. Click a node's circle to expand or collapse its branch, click its label to jump to that section, and drag or scroll to pan and zoom.
